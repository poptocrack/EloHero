    ...
    <header className="sticky top-4 z-50 mb-12 flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-6 py-4 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <picture>
            <source srcSet="/app-icon-140.avif" type="image/avif" />
            <source srcSet="/app-icon-140.webp" type="image/webp" />
            <img
              src="/app-icon-140.png"
      ...
      <nav className="hidden items-center gap-6 text-sm font-semibold text-slate-600 lg:flex">
        {navTargets.map(({ key, href }) => (
          <a key={key} href={href} className="transition-colors hover:text-primary">
            {t(`nav.${key}`)}
          </a>
        ))}
      </nav>
      <div className="flex items-center gap-3">
        <LanguageSwitcher />
        <Button size="sm" className="hidden text-sm lg:inline-flex" asChild>
          <a href="#stores" aria-label={t('hero.ctaPrimary')}>
            {t('hero.ctaPrimary')}
          </a>
        </Button>
      </div>
