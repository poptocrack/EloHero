...
exports.addMember = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        await db_1.db.collection('members').doc(`${virtualMemberUid}_${groupId}`).set({
...
exports.mergeMember = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const realMemberDoc = await db_1.db.collection('members').doc(realMemberDocId).get();
        const virtualMemberDoc = await db_1.db.collection('members').doc(virtualMemberDocId).get();
        ...
        for (const seasonDoc of seasonsSnapshot.docs) {
            ...
            const virtualRatingRef = db_1.db.collection('ratings').doc(`${seasonId}_${virtualUserId}`);
            const realRatingRef = db_1.db.collection('ratings').doc(`${seasonId}_${realUserId}`);
        ...
        for (const participantDoc of participantsSnapshot.docs) {
            ...
            if (gameDoc.exists && gameDoc.data().groupId === groupId) {
                ...
                const newParticipantRef = db_1.db.collection('participants').doc(`${gameId}_${realUserId}`);
        ...
        const ratingChangesSnapshot = await db_1.db
        ...
        for (const ratingChangeDoc of ratingChangesSnapshot.docs) {
            ...
            if (gameDoc.exists && gameDoc.data().groupId === groupId) {
                ...
                const newRatingChangeRef = db_1.db.collection('ratingChanges').doc(`${gameId}_${realUserId}`);
